        ...
        while wait() do
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('EndGameUI') and SaveSettings["Notifier"]["Send After End Game"] == true and SaveSettings["Notifier"]['Select Method [Type]'] == "Method 2" then
                ...
                for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.EndGameUI.BG.Rewards.Rewards.Holder.Reward:GetChildren()) do
                    if v:IsA("Model") then
                        if v.Name == "StrawHat" then
                            if tostring(v.Name) == tostring(v.Name) then CountSameItemAmount = CountSameItemAmount +1 end
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". TEST NEW : " .. tostring(v.Name) .. " + " .. tostring(CountSameItemAmount) .. "  :fire:\n"
                            CountAmount = CountAmount + 1
                        end
                    end
                end
                ...
                for i,v in pairs(Table_All_Items_New_data) do
                    if tonumber(v['Amount']) > 0 and tonumber(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) > 0 then
                        ...
                        if tonumber(v['Amount']) > 0 then
                            --warn(tostring(v['Name']) .. " +" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]")
                            TextDropLabelTest = TextDropLabelTest .. tostring(CountAmountTest) .. ". " .. tostring(v['UnitName']) .. " +" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]\n"
                        else
                            TextDropLabelTest = TextDropLabelTest .. tostring(CountAmountTest) .. ". " .. tostring(v['PortalName']) .. " Tier " .. tostring(string.gsub(i, "%D", "")) .. " +" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]\n"
                            --warn(tostring(v['Name']) .. " Tier " .. tostring(string.gsub(i, "%D", "")) .. " +" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]")
                        end
                        else
                    ...
                    ["embeds"] = {
                        {
                            ...
                            ["fields"] = {
                                ...
                                {
                                    ["name"] = ":customs: **• -> Item Drops TEST <- •** :customs:",
                                    ["value"] = "\n" .. TextDropLabelTest .. " ",
                                    ["inline"] = false
                                },
                                {
