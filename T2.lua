    ...
    spawn(function()
        while wait() do
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('EndGameUI') and SaveSettings["Notifier"]["Send After End Game"] == true and SaveSettings["Notifier"]['Select Method [Type]'] == "Method 2" then
                ...
                PlayerDataKillsNew = game:GetService("ReplicatedStorage").Remotes.GetPlayerData:InvokeServer()
                for i,v in pairs(PlayerDataKillsNew['Slots']) do
                    KillCountNew = v.Kills
                end

                KillGetit = (tostring(comma_value(KillCountNew)) - tostring(comma_value(KillCountOld)))
                KillCountTotal = tostring(comma_value(KillGetit))
                ...
                print('🟢🟢🟢 11111111 Webhook Method 2 Sending Resulte ... !!! 11111111111 🟢🟢🟢')
                local PlayerData = game:GetService('HttpService'):JSONEncode(PlayerData)
                HttpRequest({Url=SaveSettings["Notifier"]["Webhook Url"], Body=PlayerData, Method="POST", Headers=Headers})
                return
            end
