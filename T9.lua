...
if game.PlaceId == 14433762945 then
    ...
    -- Auto Reroll Multi Quirk Slot 1
    spawn(function()
        while wait() do
            if SaveSettings["Pet"]['Auto Reroll Mulit Quirk'] and tonumber(SaveSettings["Pet"]['Select Slot [Mulit Quirk]']) == 1 then
    ...
    -- Auto Reroll Multi Quirk Slot 2
    spawn(function()
        while wait() do
            if SaveSettings["Pet"]['Auto Reroll Mulit Quirk'] and tonumber(SaveSettings["Pet"]['Select Slot [Mulit Quirk]']) == 2 then
                local PetDataQuirk = Table_PetBaseData[SaveSettings["Pet"]['Select Pet [Mulit Quirk]']]
                if PetDataQuirk and PetDataQuirk['ID'] then
					local PetData = MainData:GetData("Pets", true)[PetDataQuirk['ID']]
					local PassiveSlots = PetStatHandler.GetPassiveSlots(PetData)
                    ...
                    if not table.find(SaveSettings["Pet"]['Select Rarity [Mulit Quirk]'],NowPassives2) and tonumber(SaveSettings["Pet"]['Select Slot [Mulit Quirk]']) == 2 then
						TableMulitQuirk2 = {}
						for i,v in pairs(SaveSettings["Pet"]['Select Rarity [Mulit Quirk]']) do
							TableMulitQuirk2[ Table_MulitQuirk_Base[v]['BaseName'] ] = {
								[Table_MulitQuirk_Base[v]['Tier']] = true
							}
						end
                    
                        if table.find(SaveSettings["Pet"]['Select Rarity [Mulit Quirk]'],NowPassives1) and TableMulitQuirk2[NowPassives1] then
                            TableMulitQuirk2[NowPassives1] = nil
                        end
                        if table.find(SaveSettings["Pet"]['Select Rarity [Mulit Quirk]'],NowPassives2) and TableMulitQuirk2[NowPassives2] then
                            TableMulitQuirk2[NowPassives2] = nil
                        end
