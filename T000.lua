...
if game.PlaceId == 14433762945 then
    ...
    local SaveSettings = {
        ...
        ['Pet'] = {
            ...
            ['Select Pet [Quirk2]'] = "",
            ['Select Rarity [Quirk21]'] = {},
            ['Select Rarity [Quirk22]'] = {},
            ['Select Rarity [Quirk23]'] = {},
            ['Select Slot2'] = 1,
            ['Premium Medal2'] = false,
            ['Multi Quirk'] = false,
            ['Auto Reroll Quirk2'] = false,
    ...
    local PetSetting_Quirk2 = PetCreateTap:addSection({
    ...
    PetSetting_Quirk2:addLabel({title = 'Pet Only Show After Lock'})
    StatusPetsQuirk = PetSetting_Quirk2:addLabel({title = 'Quirk : N/s'})
    SelectPetDropQuirk = PetSetting_Quirk2:addDropdown({
        title = "Select Pet [Quirk]",
        list = Table_SelePet, 
        default = SaveSettings["Pet"]['Select Pet [Quirk2]'],
        callback = function(v)
            SaveSettings["Pet"]['Select Pet [Quirk2]'] = v
            Save()
        end;
    })
    PetSetting_Quirk2:addMulitDropdown({
        title = "Select Quirk 1",
        list = Table_Quirk_Base, 
        default = SaveSettings["Pet"]['Select Rarity [Quirk21]'],
        callback = function(v)
            SaveSettings["Pet"]['Select Rarity [Quirk21]'] = v
            Save()
        end;
    })
    PetSetting_Quirk2:addMulitDropdown({
        title = "Select Quirk 2",
        list = Table_Quirk_Base, 
        default = SaveSettings["Pet"]['Select Rarity [Quirk22]'],
        callback = function(v)
            SaveSettings["Pet"]['Select Rarity [Quirk22]'] = v
            Save()
        end;
    })
    PetSetting_Quirk2:addMulitDropdown({
        title = "Select Quirk 3",
        list = Table_Quirk_Base, 
        default = SaveSettings["Pet"]['Select Rarity [Quirk23]'],
        callback = function(v)
            SaveSettings["Pet"]['Select Rarity [Quirk23]'] = v
            Save()
        end;
    })
    PetSetting_Quirk2:addDropdown({
        title = "Select Slot",
        list = {1,2}, 
        default = SaveSettings["Pet"]['Select Slot2'],
        callback = function(v)
            SaveSettings["Pet"]['Select Slot2'] = v
            Save()
        end;
    })
    PetSetting_Quirk2:addToggle({
        title = "Premium Medal",
        default  = SaveSettings["Pet"]['Premium Medal2'],
        callback = function(v)
            SaveSettings["Pet"]['Premium Medal2'] = v
            Save()
        end ,
    })
    PetSetting_Quirk2:addToggle({
        title = "Multi Quirk",
        default  = SaveSettings["Pet"]['Multi Quirk'] ,
        callback = function(v)
            SaveSettings["Pet"]['Multi Quirk'] = v
            Save()
        end ,
    })
    PetSetting_Quirk2:addToggle({
        title = "Auto Reroll Quirk",
        default  = SaveSettings["Pet"]['Auto Reroll Quirk2'] ,
        callback = function(v)
            SaveSettings["Pet"]['Auto Reroll Quirk2'] = v
            Save()
        end ,
    })
    PetSetting_Quirk2:addButton({
        title = "Refresh Pet",
        callback = function()
            local Table_SelePet = {}
            table.foreach(MainData:GetData("Pets", true),function(i,v)
            ...
            SelectPetDropQuirk.Options:Refresh(Table_SelePet,SaveSettings["Pet"]['Select Pet [Quirk2]'])
        end
    })
    ------------------------------------------------------ [[ Pet Auto Talent ]] ------------------------------------------------------
    ...
    -- Auto Reroll Quirk 3 Time
    spawn(function()
        while wait() do
            if SaveSettings["Pet"]['Auto Reroll Quirk2'] and SaveSettings["Pet"]['Multi Quirk'] then
                local PetDataQuirk2 = Table_PetBaseData[SaveSettings["Pet"]['Select Pet [Quirk2]']]
                if PetDataQuirk2 and PetDataQuirk2['ID'] then
                    local PetDataPassives2 = MainData:GetData("Pets", true)[PetDataQuirk2['ID']]['Passives']
                    local NowPassives = ""
                    if PetDataPassives2['1'] ~= nil and tonumber(SaveSettings["Pet"]['Select Slot2']) == 1 then
                        NowPassives = PassivesHandler.GetDisplayName(PetDataPassives2['1'][1],PetDataPassives2['1'][2])
                    elseif PetDataPassives2['1'] == nil and tonumber(SaveSettings["Pet"]['Select Slot2']) == 1 then
                        NowPassives = "Nil"
                    end
                    if PetDataPassives2['2'] ~= nil and tonumber(SaveSettings["Pet"]['Select Slot2']) == 2 then
                        NowPassives = PassivesHandler.GetDisplayName(PetDataPassives2['2'][1],PetDataPassives2['2'][2])
                    elseif PetDataPassives2['2'] == nil and tonumber(SaveSettings["Pet"]['Select Slot2']) == 2 then
                        NowPassives = "Nil"
                    end
                    StatusPetsQuirk.Options:ChangeText('Quirk : ' .. NowPassives)
        
                    --[[local Quirk1 = table.find(SaveSettings["Pet"]['Select Rarity [Quirk21]'],NowPassives)
                    local Quirk2 = table.find(SaveSettings["Pet"]['Select Rarity [Quirk22]'],NowPassives)
                    local Quirk3 = table.find(SaveSettings["Pet"]['Select Rarity [Quirk23]'],NowPassives)
                    if not Quirk1 or not Quirk2 or not Quirk3 then]]

                    if not table.find(SaveSettings["Pet"]['Select Rarity [Quirk21]'],NowPassives) or not table.find(SaveSettings["Pet"]['Select Rarity [Quirk22]'],NowPassives)
                    or not table.find(SaveSettings["Pet"]['Select Rarity [Quirk23]'],NowPassives) then

                        local args = {
                            [1] = MainData:GetData("Pets", true)[PetDataQuirk2['ID']]['GUID'],
                            [2] = tonumber(SaveSettings["Pet"]['Select Slot2']),
                            [3] = SaveSettings["Pet"]['Premium Medal'] and true or false,
                            [4] = 3,
                            [5] = {
                                [tostring(SaveSettings["Pet"]['Select Rarity [Quirk21]'])] = {
                                    ["1"] = true
                                },
                                [tostring(SaveSettings["Pet"]['Select Rarity [Quirk22]'])] = {
                                    ["1"] = true
                                },
                                [tostring(SaveSettings["Pet"]['Select Rarity [Quirk23]'])] = {
                                    ["1"] = true
                                }
                            }
                        }
    ...
    spawn(function()
        while wait() do
            if SaveSettings["Pet"]['Auto Reroll Quirk'] and not SaveSettings["Pet"]['Multi Quirk'] then
    ...
    print(SaveSettings["Raids"]["Select Raids [World]"] or (DateWorld[SaveSettings["Raids"]["Select Raids [World]"]].WorldName))
    ...
    spawn(function()
        while wait() do
            ...
            elseif SaveSettings["MaxDamageChallenge"]['Auto Farm'] and game:GetService("Workspace").Worlds:FindFirstChild("MaxDamageChallenge") then
                ...
                if MaxDamageChallenge and MaxDamageChallenge:FindFirstChild('MaxDamageChallengeChest') and MaxDamageChallenge:FindFirstChild('MaxDamageChallengeChest'):FindFirstChild('HumanoidRootPart') then
