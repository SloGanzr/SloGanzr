...
if game.PlaceId == 14433762945 then
        ...
        while wait() do
            if SaveSettings["Egg"]['Auto Open Egg'] then
                ...
                elseif EggPart ~= "API ERROR" and not string.find(tostring(EggPart),"Not Found IsLand!!") and (Character:GetModelCFrame().Position - EggPart:GetPivot().Position).Magnitude <= 10 then
                    ...
                    OpenEgg(EggPart)
                elseif game:GetService("Players").LocalPlayer.PlayerGui.OrbGui.Normal.Buttons.AutoOpenButton.Button.Visible == true and SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] then
                    wait(5)
    ...
    --Auto Click open Egg [InGame]
    --[[spawn(function()
        while wait() do
            if game:GetService("Players").LocalPlayer.PlayerGui.OrbGui.Normal.Buttons.AutoOpenButton.Button.Visible == true and SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] then
                wait(5)
                ClickButton(game:GetService("Players").LocalPlayer.PlayerGui.OrbGui.Normal.Buttons.AutoOpenButton.Button)
            end
        end
            ...
            if SaveSettings["Pet"]['Auto Reroll Multi Quirk'] then
                ...
                if PetDataQuirk and PetDataQuirk['ID'] then
                    ...
                    if not table.find(SaveSettings["Pet"]['Select Rarity [Multi Quirk]'],NowPassives1) and tonumber(SaveSettings["Pet"]['Select Slot [Multi Quirk]']) == 1 then
                        ...
                        if table.find(SaveSettings["Pet"]['Select Rarity [Multi Quirk]'],NowPassives1) and TableMultiQuirk[NowPassives1] then
                            TableMultiQuirk[NowPassives1] = nil
                        end
                       
                        if table.find(SaveSettings["Pet"]['Select Rarity [Multi Quirk]'],NowPassives2) and TableMultiQuirk[NowPassives2] then
                            TableMultiQuirk[NowPassives2] = nil
                        end
					...
					if not table.find(SaveSettings["Pet"]['Select Rarity [Multi Quirk]'],NowPassives2) and tonumber(SaveSettings["Pet"]['Select Slot [Multi Quirk]']) == 2 then
                        ...
                        table.foreach(TableMultiQuirk,warn)
