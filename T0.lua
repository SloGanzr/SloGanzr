...
if game.PlaceId == 14433762945 then
    ...
    local PetSetting_CosmicSummon = PetCreateTap:addSection({
    ...
    ------------------------------------------------------ [[ Pet Cosmic Summon ]] ------------------------------------------------------
    StatusCosmicSummon = PetSetting_CosmicSummon:addLabel({title = 'Cosmic Summon : N/s [ 0 ]'})
    PetSetting_CosmicSummon:addDropdown({
    ...
    PetSetting_CosmicSummon:addToggle({
        ...
        while wait() do
            if SaveSettings["Egg"]['Auto Open Egg'] then
                ...
                elseif EggPart ~= "API ERROR" and not string.find(tostring(EggPart),"Not Found IsLand!!") and (Character:GetModelCFrame().Position - EggPart:GetPivot().Position).Magnitude <= 10 then
                    ...
                    if SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] and game:GetService("Players").LocalPlayer.PlayerGui.OrbGui.Normal.Buttons.AutoOpenButton.Button.Visible == true then
                        wait(5)
            ...
            if SaveSettings["Pet"]['Auto Reroll Quirk'] then
                ...
                if PetDataQuirk and PetDataQuirk['ID'] then
                    ...
                    elseif PetDataPassives['1'] == nil and tonumber(SaveSettings["Pet"]['Select Slot']) == 1 then
                        NowPassives = "Nil .. Wait a Quirk Reroll .."
                    end
            ...
            if SaveSettings["Pet"]['Auto Reroll Mulit Quirk'] then
                ...
                if PetDataQuirk and PetDataQuirk['ID'] then
					...
					elseif PetDataPassives['1'] == nil then
						NowPassives1 = "Nil .. Wait a Quirk Reroll .."
					end
					...
					elseif PetDataPassives['2'] == nil then
						NowPassives2 = "Nil .. Wait a Quirk Reroll .."
					end
                    ...
                    if not table.find(SaveSettings["Pet"]['Select Rarity [Mulit Quirk]'],NowPassives1) and tonumber(SaveSettings["Pet"]['Select Slot [Mulit Quirk]']) == 1 then
                        ...
                        --fixtwinQuirk
                        if table.find(TableMulitQuirk[NowPassives2],NowPassives1) and TableMulitQuirk[NowPassives1] then
                            TableMulitQuirk[NowPassives1] = nil
                        end
                        if table.find(TableMulitQuirk[NowPassives1],NowPassives2) and TableMulitQuirk[NowPassives2] then
                            TableMulitQuirk[NowPassives2] = nil
                        end
                        --[[if NowPassives1 == TableMulitQuirk[NowPassives2] then
                            TableMulitQuirk[NowPassives2] = {
                                [NowPassives1] = nil
                            }
                        end
                        if NowPassives2 == TableMulitQuirk[NowPassives1] then
                            TableMulitQuirk[NowPassives1] = {
                                [NowPassives2] = nil
                            }
                        end]]
					...
					if not table.find(SaveSettings["Pet"]['Select Rarity [Mulit Quirk]'],NowPassives2) and tonumber(SaveSettings["Pet"]['Select Slot [Mulit Quirk]']) == 2 then
						TableMulitQuirk = {}
						for i,v in pairs(SaveSettings["Pet"]['Select Rarity [Mulit Quirk]']) do
							TableMulitQuirk[ Table_MulitQuirk_Base[v]['BaseName'] ] = {
								[Table_MulitQuirk_Base[v]['Tier']] = true
    ...
    print(tonumber(SaveSettings["Pet"]['Select Slot']))
    print(tostring(SaveSettings["Pet"]['Select Slot']))
    print(SaveSettings["Pet"]['Select Slot'])
    warn("_______________")
    print(tonumber(SaveSettings["Pet"]['Select Slot [Mulit Quirk]']))
    print(tostring(SaveSettings["Pet"]['Select Slot [Mulit Quirk]']))
    print(SaveSettings["Pet"]['Select Slot [Mulit Quirk]'])
