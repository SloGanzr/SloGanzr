...
if game.PlaceId == 14433762945 then
    ...
    local EggItems_CosmicSummon = EggCreateTap:addSection({
        title = "Eggs & Items - Cosmic Summon [Capsule]"
    })
    local EggItems_Items = EggCreateTap:addSection({
        title = "Eggs & Items - Items"
    })
        ...
        while wait() do
            if SaveSettings["Pet"]['Auto Reroll Quirk'] then
                ...
                if PetDataQuirk and PetDataQuirk['ID'] then
                    ...
                    local PetDataTEST = MainData:GetData("Pets", true)[PetDataQuirk['ID']]['DisplayName']

                    print(PetDataQuirk)
                    print(PetDataPassives)
                    print(PetDataTEST)
