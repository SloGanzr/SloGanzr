...
if game.PlaceId == 14433762945 then
    ...
    --Click Auto open Egg In Game
    spawn(function()
        while wait() do
            if SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] and LocalPlayer.PlayerGui:FindFirstChild('OrbGui'):FindFirstChild('Normal'):FindFirstChild('Buttons'):FindFirstChild('AutoOpenButton').Button.Visible == true then
                ClickButton(LocalPlayer.PlayerGui.OrbGui.Normal.Buttons.AutoOpenButton.Button)
            end
            repeat wait() until not SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] and LocalPlayer.PlayerGui:FindFirstChild('OrbGui'):FindFirstChild('AutoOpen').CloseButton.Visible == true
        end
    end)
    --Disable Click Auto open Egg In Game
    spawn(function()
        while wait() do
            if not SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] and LocalPlayer.PlayerGui:FindFirstChild('OrbGui'):FindFirstChild('AutoOpen').CloseButton.Visible == true then
                ClickButton(LocalPlayer.PlayerGui.OrbGui.AutoOpen.CloseButton)
            end
            repeat wait() until not SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] and LocalPlayer.PlayerGui:FindFirstChild('OrbGui'):FindFirstChild('Normal'):FindFirstChild('Buttons').AutoOpenButton.Visible == true
        end
    end)
