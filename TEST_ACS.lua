...
local PandaAuth = tryLoadURL('https://pandadevelopment.net/servicelib?service='..ServiceID..'&core='..LibType..'&param='..LibVersion)
...
if table.find(PlaceId_ACS,game.PlaceId) then
    ...
    local SaveSettings = {
        ...
        ["Event Easter"] = {
            ...
            ['Select Farm Method2'] = "Mini Boss",
            ['Auto Farm Easter'] = false,
    ...
    function CollectCoinCP()
        for i,v in pairs(game:GetService("Workspace").Effects:GetChildren()) do
            if v.Name == "CurrencyDrop" and v:FindFirstChild("CoinPickup") then
                if v:FindFirstChild("CoinPickup") then
                    v.CoinPickup:Fire()
                    v:FindFirstChild("CoinPickup"):Destroy()
                    repeat wait() until not v:FindFirstChild("CoinPickup")
                end
            end
        end
    end
    ...
    function CollectImageDropCP()
        for i,v in pairs(game:GetService("Workspace").Effects:GetChildren()) do
            if v.Name == "ImageDrop" and v:FindFirstChild("CoinPickup") then
                if v:FindFirstChild("CoinPickup") then
                    v.CoinPickup:Fire()
                    v:FindFirstChild("CoinPickup"):Destroy()
                    repeat wait() until not v:FindFirstChild("CoinPickup")
    ...
    RarityList[6] = "Secret"
    RarityList[7] = "Secret2"

    local ItemsInventory = MainData:GetData("Items", true)
    for i,v_Rarity in pairs(RarityList) do
        TextRarity = v_Rarity == "Secret" and "Cosmic" or v_Rarity == "Secret2" and "Astral" or v_Rarity
    ...
    table.foreach(ItemsInventory,function(index,value)
        if ItemStats[index].Skin then
            TextRarity = ItemStats[index].Rarity == "Secret" and "Cosmic" or ItemStats[index].Rarity == "Secret2" and "Astral" or ItemStats[index].Rarity
    ...
    -- Event
    --FarmEvent
    if MainData:GetData("EasterCoins", true) then
        local EventCreateTap = UI:addPage({
            title = "• Event",
            icon = 1
        })
        local AutoFarmHalloween_Event = EventCreateTap:addSection({
            title = "Auto Farm Easter"
        })

        AutoFarmHalloween_Event:addDropdown({
            title = "Select Farm Method (Attack 1st)",
            list = {"Mon","Mini Boss", "Boss"}, 
            default = SaveSettings["Event Easter"]['Select Farm Method'],
            callback = function(v)
                SaveSettings["Event Easter"]['Select Farm Method'] = v
                Save()
            end;
        })

        AutoFarmHalloween_Event:addDropdown({
            title = "Select Farm Method (Attack 2nd)",
            list = {"Mon","Mini Boss", "Boss"}, 
            default = SaveSettings["Event Easter"]['Select Farm Method2'],
            callback = function(v)
                SaveSettings["Event Easter"]['Select Farm Method2'] = v
                Save()
            end;
        })
        ...
        AutoFarmHalloween_Event:addToggle({
            title = "Auto Farm Easter",
              default = SaveSettings["Event Easter"]['Auto Farm Easter'],
            callback = function(v)
                SaveSettings["Event Easter"]['Auto Farm Easter'] = v
                Save()
            end ,
        })
        ...
        local AutoOpenOrbs_Event = EventCreateTap:addSection({
            title = "Auto Open Orbs - Easter"
        })

        AutoOpenOrbs_Event:addDropdown({
            title = "Select Amount Hatch",
            list = {1,3,10}, 
            default = SaveSettings["Event Easter"]['Select Amount Hatch'],
            callback = function(v)
                SaveSettings["Event Easter"]['Select Amount Hatch'] = v
                Save()
            end;
        })
        AutoOpenOrbs_Event:addToggle({
            title = "Auto Open Orbs",
            default = SaveSettings["Event Easter"]['Auto Open Orbs'],
            callback = function(v)
                SaveSettings["Event Easter"]['Auto Open Orbs'] = v
                Save()
            end ,
        })
        spawn(function()
            while wait() do
                if SaveSettings["Event Easter"]['Auto Open Orbs'] then
                    local args = {
                        [1] = "ChristmasOrb",
                        [2] = tonumber(SaveSettings["Event Easter"]['Select Amount Hatch'])
                    }
    ...
    for i,v in pairs(RarityList) do
        SkinRarity = v == "Secret" and "Cosmic" or v == "Secret2" and "Astral" or v
    ...
    Floor_Dungeons:addLabel({title = "Clear All Flootr : Clear all enemy in floor and go next floor"})
    ...
    Orb_Dungeons:addLabel({title = "Clear All Floor : Clear all enemy in floor and Auto Open Orbs\nFind Orb Go to : if find Orb to Auto Open Orbs Ignore Enemy"})
    ...
    Auto_Dungeons:addToggle({
        title = "Go On The Head [Mob]",
        default  = SaveSettings["Dungeons"]['Go On The Head [MobD]'] ,
        callback = function(v)
            SaveSettings["Dungeons"]['Go On The Head [MobD]'] = v
            Save()
        end ,
    })
    Auto_Dungeons:addToggle({
    ...
    function Event2(Mode)
        if Mode == "GetBossType2" then
            local Table_Mon_Event2 = {}
            for i,v in pairs(game:GetService("Workspace").Worlds:GetChildren()) do World = v.Name end
            if game:GetService("Workspace").Worlds:FindFirstChild(World):FindFirstChild('Enemies') then
                for i,v in pairs(game:GetService("Workspace").Worlds:FindFirstChild(World).Enemies:GetChildren()) do
                    if v:FindFirstChild("HumanoidRootPart") and v:GetAttribute("Health") > 0 then
                        local EnemyStatsMon2 = EnemyStats[v.Name]
                        if EnemyStatsMon2 and EnemyStatsMon2.Boss then
                            if not Table_Mon_Event2[v.Name] then
                                Table_Mon_Event2[v.Name] = {
                                    BaseName = v.Name,
                                    DisplayName = EnemyStatsMon2.DisplayName,
                                    Type = "Boss",
                                    Count = 1
                                }
                            elseif Table_Mon_Event2[v.Name] then
                                Table_Mon_Event2[v.Name]['Count'] = Table_Mon_Event2[v.Name]['Count'] + 1
                                if Table_Mon_Event2[v.Name]['Count'] > 1 then
                                    Table_Mon_Event2[v.Name]['Type'] = "Mini Boss"
                                end
                            end
                        elseif EnemyStatsMon2 and not EnemyStatsMon2.Boss then
                            if not Table_Mon_Event2[v.Name] then
                                Table_Mon_Event2[v.Name] = {
                                    BaseName = v.Name,
                                    DisplayName = EnemyStatsMon2.DisplayName,
    ...
    function CheckDungeons(Mode,NameEggs)
        ...
        if Mode == "Eggs" then
            for i,Dungeons in pairs(game:GetService("Workspace").Worlds.Dungeons:GetChildren()) do
                if Dungeons:FindFirstChild('ZonesCompleted') and Dungeons:FindFirstChildOfClass("Model") then
        ...
        elseif Mode == "Chest" then
			for i,Dungeons in pairs(game:GetService("Workspace").Worlds.Dungeons:GetChildren()) do
                if Dungeons:FindFirstChild('ZonesCompleted') and Dungeons:FindFirstChildOfClass("Model") then
                   	for i,v in next , Dungeons:GetChildren() do
						if v:GetAttribute('ZoneRan') and v:FindFirstChild('Chests') and v:FindFirstChild('Chests'):FindFirstChild('DungeonChest') and v:FindFirstChild('Chests'):FindFirstChild('DungeonChest'):FindFirstChild('HumanoidRootPart') and v:FindFirstChild('Chests'):FindFirstChild('DungeonChest'):FindFirstChild('HumanoidRootPart'):FindFirstChild('ChestPrompt') then
                            return v:FindFirstChild('Chests'):FindFirstChild('DungeonChest'):FindFirstChild('HumanoidRootPart'):FindFirstChild('ChestPrompt')
                        end
                    end
                end
            end
        elseif Mode == "Shrine" then
            for i,Dungeons in pairs(game:GetService("Workspace").Worlds.Dungeons:GetChildren()) do
                if Dungeons:FindFirstChild('ZonesCompleted') and Dungeons:FindFirstChildOfClass("Model") then
                   for i,v in next , Dungeons:GetChildren() do
                        if v:GetAttribute('ZoneRan') and v:GetAttribute('RoomType') == "ShrineRoom" and v:FindFirstChild("Shrines"):FindFirstChild("Shrine"):FindFirstChild("0") then
							for i,FindShrine in ipairs(v:FindFirstChild("Shrines"):FindFirstChild("Shrine"):FindFirstChild("0"):GetDescendants()) do
								if FindShrine.Name == "ShrinePrompt" then
									return FindShrine
								end
							end
                        end
                ...
                if k:GetAttribute('ZoneRan') then
                    if #game:GetService("Workspace").Worlds.Dungeons.Enemies:GetChildren() ~= 0 then
                        for i,v in pairs(game:GetService("Workspace").Worlds:FindFirstChild("Dungeons").Enemies:GetChildren()) do  
                            if v:FindFirstChild("HumanoidRootPart") and v:GetAttribute("Health") > 0 and v:GetAttribute("Invulnerable") ~= true and (v:GetModelCFrame().Position - k:GetModelCFrame().Position).Magnitude <= 1000 then
    ...
    spawn(function()
        while wait() do
            ...
            wait(1.5)
            if SaveSettings["Event Easter"]['Auto Farm Easter'] and MainData:GetData("EasterCoins", true) and not game:GetService("Workspace").Worlds:FindFirstChild("ShadowWorld") then
                    ...
                    for j,k in next , DungeonsFloor:GetChildren() do
                        if k:GetAttribute('ZoneRan') and SaveSettings["Dungeons"]['Auto Dungeon'] then
                            if CheckDungeons("CheckClearFloor") > 0 then
                                    ...
                                    if SaveSettings["Dungeons"]['Auto Dungeon'] and v:FindFirstChild("HumanoidRootPart") and v:GetAttribute("Health") > 0 and v:GetAttribute("Invulnerable") ~= true and (v:GetModelCFrame().Position - k:GetModelCFrame().Position).Magnitude <= 300 then
                                        if not Dungeons.ZonesCompleted:FindFirstChild(k.Name) and k:FindFirstChild('ZoneCompleteModels') and k:GetAttribute('RoomType') == 'EndingRoom' then
                                            repeat wait()
                                                ...
                                                table.foreach(CheckPet('GetPet'),function(a,b)
                                                    if SaveSettings["Dungeons"]['Go On The Head [MobD]'] == true or SaveSettings["Dungeons"]['Go On The Head [MobD]'] == 'true' and b:FindFirstChild("Target") and (b.Target.Value == nil or b.Target.Value ~= v ) then
                                                        Character.HumanoidRootPart:PivotTo(v:GetModelCFrame() * CFrame.new(0,30,5))
                                                        SendPetOneTraget(b,v)
                                                    elseif SaveSettings["Dungeons"]['Go On The Head [MobD]'] == false or SaveSettings["Dungeons"]['Go On The Head [MobD]'] == 'false' and b:FindFirstChild("Target") and (b.Target.Value == nil or b.Target.Value ~= v ) and (Character:GetModelCFrame().Position - v:GetPivot().Position).Magnitude > 1000 then
                                                        Character.HumanoidRootPart:PivotTo(v:GetModelCFrame() * CFrame.new(0,5,5))
                                                    elseif SaveSettings["Dungeons"]['Go On The Head [MobD]'] == false or SaveSettings["Dungeons"]['Go On The Head [MobD]'] == 'false' and b:FindFirstChild("Target") and (b.Target.Value == nil or b.Target.Value ~= v ) and (Character:GetModelCFrame().Position - v:GetPivot().Position).Magnitude <= 1000 then
                                                        SendPetOneTraget(b,v)
                                                    end
                                                end)
                                ...
                                if SaveSettings["Dungeons"]['Auto Collect Shrine'] and CheckDungeons("Chest") ~= "Not Found" then
            ...
            elseif SaveSettings["Event Easter"]['Auto Farm Easter'] and MainData:GetData("EasterCoins", true) and game:GetService("Workspace").Worlds:FindFirstChild("ShadowWorld") then
                repeat wait() until not LocalPlayer.PlayerGui:FindFirstChild('TeleportGui')
                if game:GetService("Workspace").Worlds:FindFirstChild("ShadowWorld") then
                    ...
                    for i,v in pairs(game:GetService("Workspace").Worlds:GetChildren()) do World = v.Name end
                    for i,v in pairs(game:GetService("Workspace").Worlds:FindFirstChild(World).Enemies:GetChildren()) do
                        if SaveSettings["Event Easter"]['Auto Farm Easter'] and v:FindFirstChild("HumanoidRootPart") and v:GetAttribute("Health") > 0 and Event('GetBossType')[v.Name]['Type'] == SaveSettings["Event Easter"]['Select Farm Method'] then 
                            repeat wait()
                                table.foreach(CheckPet('GetPet'),function(a,b)
                                    if SaveSettings["Switch Team"]["Auto Switch Team [Noraml]"] and TableTeamData[SaveSettings["Switch Team"]['Select Team [Noraml]']] then
                                        SetEquippedTeam(MainData:GetData("Teams", true)[TableTeamData[SaveSettings["Switch Team"]['Select Team [Noraml]']]])
                                    end 
                                    if SaveSettings["Event Easter"]['Auto Farm Easter'] and b:FindFirstChild("Target") and b.Target.Value == nil and (Character:GetModelCFrame().Position - v:GetPivot().Position).Magnitude <= 150 then
                                        SendPetOneTraget(b,v)
                                    elseif SaveSettings["Event Easter"]['Auto Farm Easter'] and b:FindFirstChild("Target") and b.Target.Value == nil and (Character:GetModelCFrame().Position - v:GetPivot().Position).Magnitude > 150 then
                                        Character:PivotTo(v:GetPivot() * CFrame.new(0,3,0))
                                    end
                                end)
                            until v:GetAttribute("Health") <= 0 or not v.Parent or not SaveSettings["Event Easter"]['Auto Farm Easter']
                        elseif SaveSettings["Event Easter"]['Auto Farm Easter'] and v:FindFirstChild("HumanoidRootPart") and v:GetAttribute("Health") > 0 and Event2('GetBossType2')[v.Name]['Type'] == SaveSettings["Event Easter"]['Select Farm Method2'] then 
                            repeat wait()
                                table.foreach(CheckPet('GetPet'),function(a,b)
                                    if SaveSettings["Switch Team"]["Auto Switch Team [Noraml]"] and TableTeamData[SaveSettings["Switch Team"]['Select Team [Noraml]']] then
                                        SetEquippedTeam(MainData:GetData("Teams", true)[TableTeamData[SaveSettings["Switch Team"]['Select Team [Noraml]']]])
    ...
    _G.NoClip = game:GetService("RunService").Heartbeat:Connect(function()
        ...
        if game:GetService("Players").LocalPlayer.Character:FindFirstChild("Humanoid") then
            if SaveSettings["MaxDamageChallenge"]['Auto Farm'] and game:GetService("Workspace").Worlds:FindFirstChild("MaxDamageChallenge") then
                BodyVelocity()
            elseif SaveSettings["Dungeons"]['Auto Dungeon'] and SaveSettings["Dungeons"]['Go On The Head [MobD]'] and game:GetService("Workspace").Worlds:FindFirstChild("Dungeons") then
                BodyVelocity()
