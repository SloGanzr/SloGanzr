        ...
        while wait() do
            if SaveSettings["Egg"]['Auto Open Egg'] then
                ...
                elseif EggPart ~= "API ERROR" and not string.find(tostring(EggPart),"Not Found IsLand!!") and (Character:GetModelCFrame().Position - EggPart:GetPivot().Position).Magnitude <= 10 then
                    ...
                    if not SaveSettings["Egg"]['Click Auto Open Eggs [in Game]'] and game:GetService("Players").LocalPlayer.PlayerGui.OrbGui.AutoOpen.CloseButton.Visible == true then
                        ClickButton(game:GetService("Players").LocalPlayer.PlayerGui.OrbGui.AutoOpen.CloseButton)
                    end
                end
            end
        end
