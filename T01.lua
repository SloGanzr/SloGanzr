    ...
    spawn(function()
        while wait() do
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('EndGameUI') and SaveSettings["Notifier"]["Send After End Game"] == true and SaveSettings["Notifier"]['Select Method [Type]'] == "Method 2" then
                ...
                if Win_or_Lose == "Win" then Wave_Survaval =  Wave_Survaval .. " 😈" end
                if Win_or_Lose == "Defeat" then Wave_Survaval = Wave_Survaval .. " 😖" end

                local Iime_Survaval = tostring(FormatTime(workspace.DistributedGameTime))
                if Win_or_Lose == "Win" then Iime_Survaval =  Iime_Survaval .. " ⌛" end
                if Win_or_Lose == "Defeat" then Iime_Survaval = Iime_Survaval .. " ⏳" end
