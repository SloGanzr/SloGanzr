    ...
    spawn(function()
        while wait() do
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('EndGameUI') and SaveSettings["Notifier"]["Send After End Game"] == true and SaveSettings["Notifier"]['Select Method [Type]'] == "Method 2" then
                ...
                ------------------------------------ in webhook ------------------------------------
                ------------------------------------------------------------------------------------
                if game:GetService("Players").LocalPlayer.PlayerGui.EndGameUI.BG.Buttons.Leave.Visible == true then
                    local TextDropLabelTestTest = ""
                    local CountAmountTestTest = 1
                    for i,v in pairs(get_inventory_items()) do
                    ...
                    for i,v in pairs(Table_All_Items_New_data) do
                        if tonumber(v['Amount']) > 0 and tonumber(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) > 0 then
                            if tonumber(v['Amount']) > 0 and tonumber(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) > 0 then
                                TextDropLabelTestTest = TextDropLabelTestTest .. tostring(CountAmountTestTest) .. ". " .. tostring(tostring(v['Name'])) .. " + " .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]\n"
                                --warn(tostring(CountAmountTestTest) .. ". " .. tostring(tostring(v['Name'])) .. " +" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]\n")
                                CountAmountTestTest = CountAmountTestTest + 1
                            end
                        end
                    end

                    if TextDropLabelTestTest == "" then
                        TextDropLabelTestTest = "Not Have Items Drops"
                    end
                end
                ------------------------------------------------------------------------------------
                ------------------------------------ in webhook ------------------------------------
