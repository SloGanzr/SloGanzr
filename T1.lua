        ...
        while wait() do
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('EndGameUI') and SaveSettings["Notifier"]["Send After End Game"] == true and SaveSettings["Notifier"]['Select Method [Type]'] == "Method 2" then
                ...
                if SaveSettings["Notifier"]["Send Unit Kills"] then
                    ...
                    for i,v in pairs(PlayerDataKills['Slots']) do
                        if table.find(SaveSettings["Notifier"]['Select Unit'],tostring(i)) then
                            TextUnitKills = TextUnitKills .. "U" .. tostring(CountAmountUnit) .. ". " .. tostring(v.Value) .. "[Lv." .. tostring(comma_value(v.Level)) .. "] | Kill : " .. tostring(comma_value(v.Kills)) .. " ⚔️\n"
                            CountAmountUnit = CountAmountUnit + 1
                        end
                    end
                else
                ...
                if Win_or_Lose == "Defeat" then Win_or_Lose = "Defeat <:L_~1:1182432028080226374>" end

                local Wave_Survaval = tostring(comma_value(game:GetService("ReplicatedStorage").Wave.Value))
                if Win_or_Lose == "Win <:W_:1175418407538139217>" then Wave_Survaval =  Wave_Survaval .. " 😈" end
                if Win_or_Lose == "Defeat <:L_~1:1182432028080226374>" then Wave_Survaval = Wave_Survaval .. " 😖" end

                local Iime_Survaval = tostring(FormatTime(workspace.DistributedGameTime))
                if Win_or_Lose == "Win <:W_:1175418407538139217>" then Iime_Survaval =  Iime_Survaval .. " ⌛" end
                if Win_or_Lose == "Defeat <:L_~1:1182432028080226374>" then Iime_Survaval = Iime_Survaval .. " ⏳" end
    ...
    spawn(function()
        while wait() do
            if SaveSettings["SaveSetting"]['Auto TP to Zone'] and game.PlaceId ~= 12886143095 then
                ...
                local Random_Num_x = math.random(1,10)
                local Random_Num_y = math.random(1,1)
                local Random_Num_z = math.random(1,10)
