...
if game.PlaceId == 14433762945 then
    ...
    local TableRaids2,Select_Raids2 = {},{}
    local WorldDate2 = require(game:GetService("ReplicatedStorage").ModuleScripts.Config.GameEvents)
    for i,v in pairs(WorldDate2) do
        if not v.NoRaid then
            if not v.Hidden then
                table.insert(TableRaids2,v.DisplayName)
            end
        end
    end
    table.foreach(Table_World,function(a,b)
        if table.find(TableRaids2,b.Name) then
            table.insert(Select_Raids2,b.Name)
        end
    end)
    local Select_Raids3 = {Select_Raids,Select_Raids2}
