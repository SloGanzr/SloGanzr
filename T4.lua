...
if table.find(PlaceId_ALS,game.PlaceId) then
    ...
    local TableData = {}
    table.foreach(PlayerData['ItemData'],function(i,v)
        --warn(i,v.ItemName .. " | " .. v.Amount)
        table.insert(TableData,tostring(v))
    end)

    function get_inventory_items()
        return PlayerData['ItemData']
    end

    local Table_All_Items_Old_data = {}
    local Table_All_Items_New_data = {}
    for v2, v3 in pairs(game:GetService("ReplicatedStorage").Modules.ItemNames:GetDescendants()) do
        if v3:IsA("ModuleScript") then
            for v4, v5 in pairs(require(v3)) do
                Table_All_Items_Old_data[v4] = {}
                Table_All_Items_Old_data[v4]['ItemName'] = v5['ItemName']
                Table_All_Items_Old_data[v4]['Amount'] = 0
                Table_All_Items_New_data[v4] = {}
                Table_All_Items_New_data[v4]['ItemName'] = v5['ItemName']
                Table_All_Items_New_data[v4]['Amount'] = 0
            end
        end
    end
    for i,v in pairs(get_inventory_items()) do
        Table_All_Items_Old_data[i]['Amount'] = v
    end
    ------------------------------------------------------ [[ Function First ]] ------------------------------------------------------
    ...
    local MacroMaps = MacroMapsCreateTap:addSection({
        title = "Macro - Select Map"
    })
    ]]
    -- Macro Import
    ...
    MainPage:addToggle({
        title = "Auto Join Room [Story,Inf,Challenge]",
    ...
    ------------------------------------------------------ [[ Macro Map ]] ------------------------------------------------------
    --[[
    MacroSelect = MacroMaps:addDropdown({
        title = "Macro Maps Mode Select",
        list = {"Story","Infinite","Challenge"}, 
        default = SaveSettings['Macro']['Maps Mode'],
        callback = function(v)
            if not table.find(SaveSettings['Macro']['Maps Mode Select'],v) then
                SaveSettings['Macro']['Maps Mode Select'][v] = {}
            end
            SaveSettings['Macro']['Maps Mode'] = v
            Save()
        end;
    })
    local TableMacroMap = {}
    MacroSelect.Options:Refresh(CreateMarocList(),SaveSettings['Macro']['Macro Select']) 
    for i,Name in pairs(TableMap) do
        TableMacroMap[Name] =  MacroMaps:addDropdown({
            title = tostring(Name),
            list = MacroSelect, 
            default = SaveSettings['Macro']['Macro Select'],
            callback = function(v)
                SaveSettings['Macro']['Maps Mode Select'][SaveSettings['Macro']['Maps Mode'][Name] = v
                Save()
            end;
        })
    end
    function UpdateTableMacroMap()
        for i,v in pairs(TableMacroMap) do
            v.Options:Refresh(CreateMarocList(),SaveSettings['Macro']['Macro Select']) 
        end
    end

    ]]
    -- workspace.Map.MapName
    -- workspace.Map.MapDifficulty

    -- game:GetService("Players").LocalPlayer.PlayerGui.MainUI.Wave 
    -- inf Mode : Wave 5


    -- game:GetService("ReplicatedStorage").Challenge
    ------------------------------------------------------ [[ Import Macro ]] ------------------------------------------------------
    ImportFileName = "None"
        ...
        while wait() do
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('EndGameUI') and SaveSettings["Notifier"]["Send After End Game"] == true then
                ...
                for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.EndGameUI.BG.Rewards.Rewards.Holder:GetChildren()) do
                    if v:IsA("Frame") then
                        if v.Image.Image == "rbxassetid://16088705279" then
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". Emeralds + " .. v.Text.Text .. "\n"
                            CountAmount = CountAmount + 1
                        elseif v.MainText.Text == "EXP" then
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". EXP + " .. v.Text.Text .. "\n"
                            CountAmount = CountAmount + 1
                        ...
                        or v.Text.Text == "Valentines Portal" then
                            TextDropLabel = TextDropLabel .. tostring(CountAmount) .. ". Get Portal " .. v.Text.Text .. " + 1\n"
                            CountAmount = CountAmount + 1
                        else
               ...
               --TEST 
                ...
                local AmountAmountTest = 1
                for i,v in pairs(get_inventory_items()) do
                    Table_All_Items_New_data[i]['Amount'] = v
                end
                    ...
                    if v['Amount'] > 0 and (v['Amount'] - Table_All_Items_Old_data[i]['Amount']) > 0 then
                        if v['Amount'] then
                            if string.gsub(i, "%D", "") == "" then
                                TextDropLabelTEST = TextDropLabelTEST .. tostring(AmountAmountTest) .. ". " .. tostring(v['ItemName']) .. " : x" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]\n"
                            else
                                TextDropLabelTEST = TextDropLabelTEST .. tostring(AmountAmountTest) .. ". " .. tostring(v['ItemName']) .. " Tier " .. tostring(string.gsub(i, "%D", "")) .. " : x" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]\n"
                            end
                                AmountAmountTest = AmountAmountTest + 1
                            else
                                TextDropLabelTEST = TextDropLabelTEST .. tostring(AmountAmountTest) .. ". " .. tostring(v['ItemName']) .. " : x" .. tostring(v['Amount'] - Table_All_Items_Old_data[i]['Amount']) .. " [Total : " .. tostring(v['Amount']) .. "]\n"
                                AmountAmountTest = AmountAmountTest + 1
                        end
                    ...
                    ["embeds"] = {
                        {
                            ...
                            ['author'] = {
                                ...
                                icon_url = 'https://tr.rbxcdn.com/df6a0a50ab4bfd2c92de9af6203cd8de/150/150/Image/Png',
                            },
                            ["image"] = {
                            ...
                            ["fields"] = {
                                ...
                                },
                                {
                                    ["name"] = "Game Stats :",
                                    ["value"] = "```ini\n" .. 
                                                "• Map -> " .. tostring(workspace.Map.MapName.Value) .. " (" .. tostring(workspace.Map.MapDifficulty.Value) ..")" ..
                                                "\n• Waves Survived -> " .. tostring(comma_value(game:GetService("ReplicatedStorage").Wave.Value)) .. 
                                                "\n• Time Elapsed -> " .. tostring(comma_value(game:GetService("Players").LocalPlayer.PlayerGui.EndGameUI.BG.ElapsedTime.Text:split(" ")[3])) .. 
    ...
    spawn(function()
        while wait() do
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("EndGameUI") then
                if SaveSettings["Main"]['Auto Next'] then
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    ClickNext()
                elseif SaveSettings["Main"]['Auto Retry'] then
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    ClickRetry()
                elseif SaveSettings["Main"]['Auto Leave'] then
                    wait(tonumber(tostring(SaveSettings["Main"]["Delay [Start,Retry,Leave]"])))
                    ClickLeave()
                end 
            end
            ...
            and table.find(SaveSettings["Main"]['Select Challenge Map'],game:GetService("Workspace").TeleporterFolder.Challenge.Teleporter.ChallengeInfo.MapName.Value) and table.find(SaveSettings["Main"]['Select Challenge'],game:GetService("Workspace").TeleporterFolder.Challenge.Teleporter.ChallengeInfo.Challenge.Value) then
                ...
                if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('MapSelection') then
                    ...
                    if game:GetService("Workspace").TeleporterFolder:FindFirstChild(SaveSettings["Main"]['Select Mode']) then
                        Challenge = game:GetService("Workspace").TeleporterFolder:FindFirstChild(SaveSettings["Main"]['Select Mode']):GetChildren()
            ...
            elseif SaveSettings["Main"]['Auto Join Story'] and SaveSettings["Main"]['Select Mode'] == "Infinite" and game.PlaceId == 12886143095 then
                if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild('MapSelection') then
                    ...
                    if game:GetService("Workspace").TeleporterFolder:FindFirstChild(SaveSettings["Main"]['Select Mode']) then
                        Infinite = game:GetService("Workspace").TeleporterFolder:FindFirstChild(SaveSettings["Main"]['Select Mode']):GetChildren()
    ...
    spawn(function()
        while wait() do
        local PlaceId_ALS = {12900046592,14368918515,12886143095}
        if table.find(PlaceId_ALS,game.PlaceId) and SaveSettings["SaveSetting"]['Auto Rejoin When Error prompt [Time]'] then
            local CountCheck = 1
